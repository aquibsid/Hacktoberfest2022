...
function initializeDashboard() {
  ...
  const country_code = typeof geoplugin_countryCode === "function" ? geoplugin_countryCode() : "IN";
  ...
  async function fetchData(country) {
    ...
    try {
      ...
      const covidConfirmedRes = await fetch(
        `https://api.covid19api.com/total/country/${encodeURIComponent(country)}/status/confirmed`,
        requestOptions
      );
      ...
      const covidRecoveredRes = await fetch(
        `https://api.covid19api.com/total/country/${encodeURIComponent(country)}/status/recovered`,
        requestOptions
      );
      ...
      const covidDeathRes = await fetch(
        `https://api.covid19api.com/total/country/${encodeURIComponent(country)}/status/deaths`,
        requestOptions
      );
      ...
      if (country_name_element) {
        country_name_element.innerHTML = `Could not load data for ${user_country}`;
      }
    } finally {
